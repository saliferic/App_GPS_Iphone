        ...
        /* ⚠⚠ LES RECTANGLES NE SUFFISENT PAS, ET C'EST STRUCTUREL  (03/09/2026).
           `COUNTRY_BOXES` ci-dessous reste utile comme PRÉ-FILTRE — trois comparaisons
           rejettent 99 % des points — mais il ne peut pas trancher, et aucun réglage de
           ses bornes n'y changera rien : la France et la Belgique s'imbriquent. La boîte
           `fr` contient BRUXELLES (elle monte à 51,1°) ; la boîte `be` contient LILLE,
           Dunkerque, Valenciennes et Charleville. Resserrer l'une casse l'autre.

           CE QUE CE DÉFAUT A COÛTÉ. `_country` enregistrait quel FETCHER avait trouvé la
           station, pas OÙ elle est. À Lille, la boîte `be` déclenchait le fetcher belge,
           Overpass — qui ignore les frontières et interroge une bbox — rendait 180
           stations FRANÇAISES, et elles héritaient du label `be`, donc de l'exception
           « station étrangère sans prix, on l'affiche » (voir gasStationAffichable).
           Mesuré : 27 stations affichées avant, 56 après, dont 29 doublons français sans
           prix intercalés entre les fiches data.gouv qui, elles, portaient leur prix.

           LE CONTOUR. Frontières Belgique et Luxembourg, relevées une fois chez Nominatim
           puis simplifiées en Douglas-Peucker à 0,01° — 288 et 84 points, ~7 Ko. Tolérance
           choisie par balayage : 0,02 fait basculer Halluin en Belgique, 0,01 garde 26/26
           villes de contrôle justes, dont Halluin (FR) et Menin (BE) séparées d'un
           kilomètre, Mouscron et Tournai (BE, à 2 km de la France), Maubeuge et Longwy.
           ⚠ UN CONTOUR SIMPLIFIÉ SE TROMPE À QUELQUES CENTAINES DE MÈTRES DE LA
           FRONTIÈRE. C'est assumé : l'enjeu est de savoir quelle SOURCE interroger et
           quelle règle de prix appliquer, pas de faire du cadastre. Ne pas s'en servir
           pour autre chose.
           ⚠ Le tracé de rayon n'utilise PAS turf : `booleanPointInPolygon` ferait le même
           travail, mais cette fonction est appelée par les tests du noyau, qui chargent
           js/00 SEUL, sans le CDN. */
        const CONTOUR_BE = [
            [2.3889,51.2683],[2.5585,51.0719],[2.5740,51.0033],[2.6070,50.9912],[2.6329,50.9462],[2.5902,50.9189],
            [2.6081,50.9101],[2.6129,50.8675],[2.5990,50.8489],[2.6167,50.8480],[2.6351,50.8129],[2.6693,50.8227],
            [2.7241,50.8106],[2.7304,50.7881],[2.7830,50.7515],[2.7901,50.7258],[2.8481,50.7230],[2.9057,50.6924],
            [2.9515,50.7521],[3.0555,50.7805],[3.0882,50.7731],[3.1123,50.7945],[3.1504,50.7901],[3.2049,50.7120],
            [3.2610,50.7013],[3.2528,50.6898],[3.2652,50.6770],[3.2401,50.6586],[3.2796,50.5959],[3.2884,50.5259],
            [3.3763,50.4911],[3.4489,50.5066],[3.4744,50.5337],[3.5199,50.5236],[3.5007,50.4867],[3.6075,50.4972],
            [3.6628,50.4558],[3.6750,50.4024],[3.6573,50.3686],[3.7099,50.3032],[3.7202,50.3050],[3.7100,50.3195],
            [3.7319,50.3115],[3.7471,50.3510],[3.8458,50.3532],[3.9009,50.3270],[4.0242,50.3592],[4.0789,50.3097],
            [-7.6607,43.9922],[-7.9106,43.9794],[-8.0195,43.9517],[-8.1929,43.8857],[-8.4643,43.7392],[-9.0103,43.5258],
            [-9.4285,43.2909],[-9.5502,43.1382],[-9.5755,43.0459]
        ];

        function _pointDansContour(ring, lng, lat) {
            /* Lancer de rayon. Un point est dedans si une demi-droite partant de lui
               coupe le contour un nombre IMPAIR de fois. */
            let dedans = false;
        ...
        /* Le pays d'UN point. Rend 'be', 'lu', 'fr' ou null (hors des trois).
           ⚠ Rend le pays RÉEL, pas « la source à interroger » : c'est ce qui permet
           d'étiqueter une station trouvée par le fetcher belge mais posée en France, et
           de lui réappliquer la règle française (« sans prix = fiche morte »). */
        function paysDuPoint(lng, lat) {
            if (typeof lng !== 'number' || typeof lat !== 'number') return null;
            const dansBoite = (b) => lat >= b.minLat && lat <= b.maxLat
                                  && lng >= b.minLng && lng <= b.maxLng;
            // Luxembourg d'abord : il est inclus dans l'emprise de la boîte belge.
            if (dansBoite(COUNTRY_BOXES.lu) && _pointDansContour(CONTOUR_LU, lng, lat)) return 'lu';
            if (dansBoite(COUNTRY_BOXES.be) && _pointDansContour(CONTOUR_BE, lng, lat)) return 'be';
            /* ⚠ L'ESPAGNE SE TESTE AVANT LE REPLI FRANÇAIS. La boîte `fr` descend à
               41,3° de latitude : elle contient BARCELONE. Sans ce contour, un trajet
               catalan se déclarait français et perdait ses stations (l'ancienne union de
               rectangles masquait le défaut en rendant `fr+es`). Symétriquement, la
               boîte `es` monte à 43,8° et contient Perpignan : c'est bien le contour,
               et non l'ordre des boîtes, qui tranche. Vérifié sur Hendaye (fr) et Irún
               (es), séparées par la Bidassoa. */
            if (dansBoite(COUNTRY_BOXES.es) && _pointDansContour(CONTOUR_ES, lng, lat)) return 'es';
            if (dansBoite(COUNTRY_BOXES.fr)) return 'fr';
            return null;
        }

        function detectCountriesOnRoute(routeCoords) {
            /* ⚠ S'APPUIE SUR paysDuPoint(), PAS SUR LES RECTANGLES  (03/09/2026).
               Avec les seules boîtes, cette fonction rendait `fr+be` pour un scan à
               LILLE — donc un aller-retour Overpass complet (2,1 s mesurées, 9,0 s à
               Valenciennes) pour des stations françaises que data.gouv servait déjà avec
               leurs prix — et `fr+be` à BRUXELLES, donc une requête data.gouv vide.
               Elle rend maintenant `fr` seul à Lille et `be` seul à Bruxelles.
               L'UNION RESTE VOULUE : une bulle posée à Mouscron chevauche vraiment la
               frontière et doit interroger les deux sources. Ce qui était faux, ce
               n'était pas le principe de l'union, c'était la géométrie qui la nourrit. */
            const countries = new Set();
            const step = Math.max(1, Math.floor(routeCoords.length / 20));
            for (let i = 0; i < routeCoords.length; i += step) {
                const cc = paysDuPoint(routeCoords[i][0], routeCoords[i][1]);
                if (cc) countries.add(cc);
            }
            if (countries.size === 0) countries.add('fr');
            // Le Luxembourg n'a pas de source propre : il est servi par le fetcher belge.
